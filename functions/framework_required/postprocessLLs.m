...
%% Alternatively, you may want to compute the log-likelihoods here for all trials simultaneously 
% For example, this could be useful if you did not fit the "sd" as a free parameter but use the RMSE as SD instead  

rms = @(x) sqrt(mean(x.^2));

if ~any(strcmp('sd',fit_settings.fit_param_names))

    LL_total = LL_total + 0;                    %It may not be possible to compute log-likelihoods per trial...
    ...
    if nargout >= 2
        ...
        SD = rms(errors_all);                   %Assume the same SD for all conditions                            
        ...
        for c=1:fit_settings.num_conds
            ...
            %SD = rms(errors);                  %Assume a particular SD per condition  
